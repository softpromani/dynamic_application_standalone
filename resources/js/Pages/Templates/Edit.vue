...
/* ─── Build initial fields from existing template data ─── */
const initialFields = (props.template.fields || []).map(f => ({
    id:               f.id,
    step:             f.step,
    ...
    is_subject_field: f.is_subject_field,
    options:          f.options || [],
    system_alias:     f.system_alias || '',
    custom_entity_id: f.custom_entity_id || null,
    table_config:     f.field_type === 'table' 
        ? (() => {
            const cfg = typeof f.options === 'string' ? JSON.parse(f.options) : f.options;
            if (cfg && cfg.columns) {
                cfg.columns = cfg.columns.map(c => ({
                    ...c,
                    select_options: c.options ? c.options.join(', ') : ''
                }));
            }
            return cfg;
          })()
...
const existingStepNums = [...new Set(initialFields.map(f => f.step))].sort((a,b)=>a-b);
const initialSteps = existingStepNums.length > 0
    ? existingStepNums.map(n => ({ id: n, title: `Step ${n}` }))
    : [{ id: 1, title: 'Step 1' }];

const formSteps  = ref(initialSteps);
const nextStepId = ref(Math.max(...initialSteps.map(s => s.id)) + 1);
const activeStepId = ref(formSteps.value[0]?.id || 1);
...
const editField = (index) => {
    ...
    // Ensure table_config is handled if it's a table and not already expanded
...
const addOption  = () => { const v = newField.value._optionInput.trim(); if (v) { newField.value.options.push(v); newField.value._optionInput = ''; } };
...
const errors = ref({});
const validateMeta   = () => { errors.value = {}; if (!meta.name.trim()) errors.value.name = 'Required.'; return !errors.value.name; };
const validateFields = () => { if (!meta.fields.length) { errors.value.fields = 'Add at least one field.'; return false; } return true; };
const goToStep = (s) => { if (s === 2 && !validateMeta()) return; if (s === 3 && !validateFields()) return; wizardStep.value = s; };

const submit = () => {
    if (!validateMeta() || !validateFields()) return;
    meta.put(route('templates.update', props.template.id));
};
...
<template>
    ...
    <AdminLayout>
        <template #header>Edit Form Template</template>
        ...
        <div class="builder-page">
            ...
            <div class="wizard-progress mb-4">
                <div class="wp-track">
                    <div class="wp-step" :class="{ active: wizardStep >= 1, done: wizardStep > 1 }">
            ...
            <div v-if="wizardStep === 1" class="builder-card">
                <div class="builder-card-header"><i class="bi bi-info-circle-fill me-2"></i>Template Information</div>
                ...
                <div class="builder-card-footer text-end">
                    <button @click="goToStep(2)" class="btn btn-primary-red">Next: Build Fields <i class="bi bi-arrow-right ms-2"></i></button>
                </div>
            </div>
            ...
            <div v-if="wizardStep === 2" class="builder-layout">
                <div class="builder-left">
                    <div class="builder-card h-100">
                        <div class="builder-card-header d-flex justify-content-between align-items-center">
                            ...
                            <button @click="addFormStep" class="btn btn-xs btn-outline-light"><i class="bi bi-plus-lg me-1"></i>Add Step</button>
                        </div>
                        <div class="step-tabs">
                            <div v-for="step in formSteps" :key="step.id" class="step-tab"
                                :class="{ 'step-tab-active': activeStepId === step.id }" @click="activeStepId = step.id">
                                <div class="d-flex align-items-center justify-content-between">
                                    ...
                                    <button v-if="formSteps.length > 1" @click.stop="removeFormStep(step.id)" class="btn btn-link btn-sm text-danger p-0 ms-2"><i class="bi bi-x-circle"></i></button>
                                </div>
                            </div>
                        </div>
                        <div class="fields-list">
                            <div v-if="activeFields.length === 0" class="empty-fields-hint">
                                <i class="bi bi-arrows-expand d-block mb-2 fs-2 text-muted"></i>No fields yet.
                            </div>
                            <div v-for="(field, idx) in activeFields" :key="idx" class="field-item">
                                <div class="field-item-left">
                                    <div class="field-type-badge"><i class="bi" :class="fieldTypeIcon(field.field_type)"></i></div>
                                    <div>
                                        <div class="field-label-text">{{ field.label }}<span v-if="field.is_required" class="req-mark">*</span></div>
                                ...
                                <div class="field-item-actions">
                                    <button @click="moveField(idx, -1)" class="btn btn-icon" :disabled="idx === 0"><i class="bi bi-chevron-up"></i></button>
                                    <button @click="moveField(idx, 1)" class="btn btn-icon" :disabled="idx === activeFields.length - 1"><i class="bi bi-chevron-down"></i></button>
                                    <button @click="editField(idx)" class="btn btn-icon text-primary"><i class="bi bi-pencil"></i></button>
                                    <button @click="removeField(idx)" class="btn btn-icon text-danger"><i class="bi bi-trash"></i></button>
                                </div>
                            </div>
                        </div>
                        <div class="builder-card-footer">
                            <button @click="openPanel" class="btn btn-add-field w-100"><i class="bi bi-plus-circle me-2"></i>Add Field to {{ activeStep?.title }}</button>
                        </div>
                    </div>
                </div>
                ...
                <div class="builder-right" v-if="showPanel">
                    <div class="panel-card">
                        <div class="panel-header">
                            ...
                            <button @click="closePanel" class="btn btn-link text-white p-0"><i class="bi bi-x-lg"></i></button>
                        </div>
                        <div class="panel-body">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="field-type-grid">
                                    <div v-for="ft in FIELD_TYPES" :key="ft.value" class="ft-chip"
                                        ...
                                        <i class="bi" :class="ft.icon"></i><span>{{ ft.label }}</span>
                                    </div>
                                </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input v-model="newField.label" type="text" class="form-control" placeholder="e.g., Select Subject…" />
                            </div>
                            ...
                            <div class="mb-3" v-if="!['checkbox', 'file', 'subject', 'radio'].includes(newField.field_type)">
                                ...
                                <input v-model="newField.placeholder" type="text" class="form-control" placeholder="Optional" />
                            </div>
                            <div class="mb-3" v-if="needsOptions(newField.field_type)">
                                <label class="form-label field-label required-star">Options</label>
                                <div class="d-flex gap-2 mb-2">
                                    <input v-model="newField._optionInput" type="text" class="form-control form-control-sm" placeholder="Type option…" @keyup.enter="addOption" />
                                    <button @click="addOption" class="btn btn-sm btn-outline-primary" type="button">Add</button>
                                </div>
                                <div class="option-pills">
                                    <span v-for="(opt, oi) in newField.options" :key="oi" class="option-pill">{{ opt }}<button @click="removeOption(oi)" class="btn-remove-opt"><i class="bi bi-x"></i></button></span>
                                    <span v-if="!newField.options.length" class="text-muted small">No options yet.</span>
                                </div>
                                    ...
                                    <div class="card-body p-3">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <div v-for="(col, ci) in newField.table_config.columns" :key="ci" class="input-group input-group-sm mb-2">
                                                <input v-model="col.label" type="text" class="form-control" placeholder="Label" />
                                                ...
                                                <button @click="removeColumn(ci)" class="btn btn-outline-danger" type="button" :disabled="newField.table_config.columns.length === 1"><i class="bi bi-trash"></i></button>
                                            ...
                                            <button @click="addColumn" class="btn btn-xs btn-outline-primary mt-1" type="button"><i class="bi bi-plus-lg me-1"></i>Add Column</button>
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label small fw-bold d-block">Row Behavior</label>
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" :value="true" v-model="newField.table_config.is_dynamic" id="rowDynE" />
                                                <label class="form-check-label small" for="rowDynE">Dynamic (+ Add Row)</label>
                                            </div>
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" :value="false" v-model="newField.table_config.is_dynamic" id="rowFixedE" />
                                                <label class="form-check-label small" for="rowFixedE">Fixed Rows</label>
                                            </div>
                                        </div>
                                        <div v-if="!newField.table_config.is_dynamic" class="mb-2">
                                            <label class="form-label small fw-bold">Fixed Row Labels</label>
                                            <div class="d-flex gap-2 mb-2">
                                                <input v-model="newField._fixedRowInput" type="text" class="form-control form-control-sm" placeholder="e.g. Matriculation" @keyup.enter="addFixedRow" />
                                                <button @click="addFixedRow" class="btn btn-sm btn-outline-primary" type="button">Add</button>
                                            </div>
                                            <div class="option-pills">
                                                <span v-for="(row, ri) in newField.table_config.fixed_rows" :key="ri" class="option-pill bg-secondary">{{ row }}<button @click="removeFixedRow(ri)" class="btn-remove-opt"><i class="bi bi-x"></i></button></span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="form-check mt-3">
                                <input class="form-check-input" type="checkbox" v-model="newField.is_required" id="reqCheckEdit" />
                                <label class="form-check-label field-label" for="reqCheckEdit">Required field</label>
                            </div>
                        </div>
                ...
                <div class="builder-right-empty" v-else>
                    <div class="empty-panel-hint">
                        ...
                        <p class="text-muted">Click <strong>Add Field</strong> to add fields.</p>
                    </div>
                </div>
            </div>
            ...
            <div v-if="wizardStep === 3" class="builder-card">
                ...
                <div class="builder-card-body">
                    ...
                    <div v-for="step in formSteps" :key="step.id" class="review-step mb-4">
                        <h6 class="review-step-title"><i class="bi bi-layers me-2"></i>{{ step.title }}<span class="step-field-count ms-2">{{ meta.fields.filter(f => f.step === step.id).length }} fields</span></h6>
                        <div class="preview-form-area">
                            <div v-if="!meta.fields.filter(f => f.step === step.id).length" class="text-muted small">No fields.</div>
                            <div v-for="(field, fi) in meta.fields.filter(f => f.step === step.id)" :key="fi" class="preview-field">
                                <label class="preview-label">{{ field.label }}<span v-if="field.is_required" class="req-mark">*</span><span class="preview-type-badge ms-2">{{ fieldTypeLabel(field.field_type) }}</span></label>
                                ...
                                <textarea v-else-if="field.field_type === 'textarea'" class="form-control form-control-sm" rows="2" disabled></textarea>
                                ...
                                <div v-else-if="field.field_type === 'table'" class="table-responsive">
                                    <table class="table table-bordered table-sm mb-0 small">
                                        <thead class="table-light">
                                            <tr>
                                                <th v-for="col in (typeof field.options === 'string' ? JSON.parse(field.options) : field.options).columns" :key="col.label">{{ col.label }}</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <tr v-if="(typeof field.options === 'string' ? JSON.parse(field.options) : field.options).is_dynamic">
                                                <td :colspan="(typeof field.options === 'string' ? JSON.parse(field.options) : field.options).columns.length" class="text-center text-muted py-2">Applicant can add multiple rows</td>
                                            </tr>
                                            <tr v-else v-for="row in (typeof field.options === 'string' ? JSON.parse(field.options) : field.options).fixed_rows" :key="row">
                                                <td><strong>{{ row }}</strong></td>
                                                <td v-for="i in (typeof field.options === 'string' ? JSON.parse(field.options) : field.options).columns.length - 1" :key="i"></td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>

                                <input v-else :type="['subject','select','radio','checkbox','table'].includes(field.field_type) ? 'text' : field.field_type" class="form-control form-control-sm" disabled />
                            </div>
                        </div>
                    </div>
                </div>
...
<style scoped>
/* Re-uses same styles as Create.vue */
...
.wp-step { display: flex; flex-direction: column; align-items: center; min-width: 80px; }
.wp-circle { width: 44px; height: 44px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 1.1rem; background: #f0f0f0; color: #aaa; border: 2px solid #e0e0e0; transition: all 0.3s; }
...
.builder-card-header { background: linear-gradient(135deg, #8b0000, #b71c1c); color: #fff; padding: 16px 24px; font-weight: 700; font-size: 0.95rem; }
...
.step-tab { padding: 12px 20px; cursor: pointer; border-left: 3px solid transparent; font-size: 0.875rem; font-weight: 500; color: #555; transition: all 0.15s; background: #fafafa; }
...
.field-item { display: flex; align-items: center; justify-content: space-between; background: #fafafa; border: 1px solid #eee; border-radius: 8px; padding: 10px 12px; margin-bottom: 8px; transition: border-color 0.15s; }
.field-item:hover { border-color: #c0392b; }
.field-item-left { display: flex; align-items: center; gap: 10px; }
.field-type-badge { width: 34px; height: 34px; background: #8b0000; color: #fff; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 0.9rem; flex-shrink: 0; }
...
.btn-add-field { background: linear-gradient(135deg, #8b0000, #c0392b); color: #fff; border: none; border-radius: 8px; padding: 10px; font-weight: 600; font-size: 0.875rem; transition: all 0.2s; }
...
.ft-chip { display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 10px 6px; border: 2px solid #e0e0e0; border-radius: 10px; cursor: pointer; font-size: 0.72rem; font-weight: 600; color: #555; text-align: center; transition: all 0.15s; background: #fafafa; gap: 4px; }
...
.option-pill { background: #8b0000; color: #fff; font-size: 0.75rem; padding: 4px 12px; border-radius: 100px; display: flex; align-items: center; gap: 4px; }
...
.btn-primary-red { background: linear-gradient(135deg, #8b0000, #c0392b); color: #fff; border: none; border-radius: 8px; padding: 9px 22px; font-weight: 600; font-size: 0.875rem; transition: all 0.2s; }
