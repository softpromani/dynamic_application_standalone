...
const form = useForm({
    job_code: props.program.job_code,
    title: props.program.title,
    description: props.program.description ?? '',
    application_start_date: props.program.application_start_date?.slice(0, 10) ?? '',
    application_end_date: props.program.application_end_date?.slice(0, 10) ?? '',
    last_payment_date: props.program.last_payment_date?.slice(0, 10) ?? '',
    is_active: props.program.is_active,
    footer_notes: props.program.footer_notes ?? '',
    is_payable: props.program.is_payable,
    custom_entity_id: props.program.custom_entity_id ?? '',
    application_types: (props.program.application_types ?? []).map(t => ({
        id: t.id,
        name: t.name,
        fee: t.fee,
    ...
    preview_config: props.program.preview_config ?? {
        show_photo: true,
        show_signature: true,
...
const addType = () => form.application_types.push({ id: null, name: '', fee: 0, fine_amount: 0 });
...
const usedValueIds = () => form.openings.map(v => v.subject_id).filter(id => id !== null);

const addVacancy = () => {
    vacancyError.value = '';
    
    if (form.custom_entity_id && !newVacancy.value.subject_id) {
        vacancyError.value = `Please select a ${selectedEntity.value?.display_name || 'value'}.`;
        return;
    }

    if (newVacancy.value.subject_id && usedValueIds().includes(Number(newVacancy.value.subject_id))) {
    ...
    form.openings.push({
        id: null,
        subject_id: newVacancy.value.subject_id ? Number(newVacancy.value.subject_id) : null,
        seats: Number(newVacancy.value.seats),
    });
...
// Reset openings if entity changes (and it's a manual change, not initial load)
watch(() => form.custom_entity_id, (newVal, oldVal) => {
    if (oldVal !== undefined) {
        // Only clear if the entity actually changed from its original value
        // But in edit mode, we might want to be careful.
        // Let's just warn or allow it.
    }
});
...
<template>
    <Head title="Edit Program Post" />

    <AdminLayout>
        <template #header>Edit Program Posting</template>

        <form @submit.prevent="submit">
            <div v-if="Object.keys(form.errors).length > 0" class="alert alert-danger py-2 small mb-3 shadow-sm border-0">
                ...
                <div class="col-lg-7 mb-3">
                    <div class="card border-0 shadow-sm mb-4">
                        ...
                        <div class="card-body">
                            <div class="row g-3">
                                <div class="col-md-4">
                                    <TextInput v-model="form.job_code" label="Program Code"
                            ...
                            <div class="row g-3 mt-1">
                                <div class="col-md-6">
                                    ...
                                    <select v-model="form.form_template_id" class="form-select form-select-sm shadow-sm" :class="{'is-invalid': form.errors.form_template_id}">
                                ...
                                <div class="col-md-6">
                                    <label class="form-label fw-semibold small text-primary">Opening Grouping (Entity)</label>
                                    <select v-model="form.custom_entity_id" class="form-select form-select-sm shadow-sm border-primary-subtle">
                                    ...
                                    <div class="form-text x-small text-muted">Changing this will hide existing openings if they don't match the new entity.</div>
                                </div>
                            </div>
                            ...
                            <div v-if="form.is_payable">
                                ...
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                    ...
                                    <button type="button" @click="addType" class="btn btn-xs btn-outline-primary rounded-pill px-3">
                                        <i class="bi bi-plus-lg me-1"></i>Add Category
                                    </button>
                                </div>

                                <div v-for="(type, idx) in form.application_types" :key="type.id ?? idx" class="row g-2 mb-3 align-items-end p-2 border rounded-3 bg-light position-relative">
                                    <div class="col-md-5">
                                    ...
                                    <div class="col-md-1">
                                        <button type="button" @click="removeType(idx)" class="btn btn-link text-danger mb-2 border-0" 
                            ...
                            <div class="row g-3 mt-3">
                                ...
                                <div class="col-md-4 d-flex align-items-center">
                                    <div class="form-check form-switch mt-3">
                                        ...
                                        <label class="form-check-label fw-bold small" for="isPayable">Fee Required</label>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card-body">
                            ...
                            <div v-else class="table-responsive rounded-3 border">
                                <table class="table table-hover align-middle mb-0">
                                    ...
                                    <tbody>
                                        <tr v-for="(v, i) in form.openings" :key="v.id ?? i">
                                            <td class="ps-3">
